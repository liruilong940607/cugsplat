...
struct DeviceSimplePinholeCameraEWA {
    ...
    glm::fvec2* __restrict__ focal_lengths;
    glm::fvec2* __restrict__ principal_points;
    glm::fmat3* __restrict__ world_to_camera_R;
    glm::fvec3* __restrict__ world_to_camera_t;
    ...
    // ctx
    Maybe<glm::fvec2> camera_point;

    inline __device__ glm::fvec2 get_focal_lengths() { 
        if (!this->focal_lengths.has_value()) {
            this->focal_lengths.set(focal_lengths[index]);
        }
        return this->focal_lengths.get();
    }

    inline __device__ glm::fvec2 get_principal_point() { 
        if (!this->principal_points.has_value()) {
            this->principal_points.set(principal_points[index]);
        }
        return this->principal_points.get();
    }

    inline __device__ glm::fmat3 get_world_to_camera_R() { 
        if (!this->world_to_camera_R.has_value()) {
            this->world_to_camera_R.set(world_to_camera_R[index]);
        }
    ...
    inline __device__ glm::fvec3 get_world_to_camera_t() { 
        if (!this->world_to_camera_t.has_value()) {
            this->world_to_camera_t.set(world_to_camera_t[index]);
        }
        return this->world_to_camera_t.get();
    }

    inline __device__ glm::fvec3 point_world_to_camera(const glm::fvec3& world_point) const {
    ...
    inline __device__ glm::fvec2 point_world_to_image(const glm::fvec3& world_point) const {
    ...
    inline __device__ glm::fmat2 jacobian_world_to_image(const glm::fvec3& world_point) const {
        auto const camera_point = this->point_world_to_camera(world_point);
    ...
    inline __device__ glm::fmat3x2 jacobian_camera_to_image(const glm::fvec3& camera_point) const {
        auto const focal_length = this->get_focal_lengths();
