...
//     cudaMemcpy(d_gaussian_in.opacity_ptr, &opacity, 1 * sizeof(float), cudaMemcpyHostToDevice);
//     cudaMalloc(&d_gaussian_in.mean_ptr, 1 * sizeof(glm::fvec3));
//     cudaMemcpy(d_gaussian_in.mean_ptr, &mean, 1 * sizeof(glm::fvec3), cudaMemcpyHostToDevice);
//     cudaMalloc(&d_gaussian_in.quat_ptr, 1 * sizeof(glm::fvec4));
//     cudaMemcpy(d_gaussian_in.quat_ptr, &quat, 1 * sizeof(glm::fvec4), cudaMemcpyHostToDevice);
//     cudaMalloc(&d_gaussian_in.scale_ptr, 1 * sizeof(glm::fvec3));
//     cudaMemcpy(d_gaussian_in.scale_ptr, &scale, 1 * sizeof(glm::fvec3), cudaMemcpyHostToDevice);
