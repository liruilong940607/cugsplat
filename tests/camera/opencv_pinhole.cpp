...
int main() {
    ...
    printf("Image Point: (%f, %f), Valid: %d\n",
           image_point.x, image_point.y, point_valid_flag);
    auto const &[ray_o, ray_d, ray_valid_flag] = projector.image_point_to_camera_ray(image_point);
    printf("Camera Ray Origin: (%f, %f, %f), Direction: (%f, %f, %f), Valid: %d\n",
           ray_o.x, ray_o.y, ray_o.z,
           ray_d.x, ray_d.y, ray_d.z, ray_valid_flag);

    auto const depth = 12.9f; // arbitrary depth
    auto const camera_point_ = ray_o + depth * ray_d;
    auto const &[image_point_, point_valid_flag_] = projector.camera_point_to_image_point(camera_point_);
    printf("Image Point (verify): (%f, %f), Valid: %d\n",
           image_point_.x, image_point_.y, point_valid_flag_);
}