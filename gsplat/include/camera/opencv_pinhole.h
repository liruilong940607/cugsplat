...
struct OpencvPinholeProjectionImpl {
    ...
    GSPLAT_HOST_DEVICE auto image_point_to_camera_ray(
        ...
        if (is_perfect) {
            ...
            auto const &[xy_, valid_flag] =
                this->compute_undistortion(uv);
            if (!valid_flag)
    ...
    GSPLAT_HOST_DEVICE auto camera_point_to_image_point_jacobian(
        const glm::fvec3 &camera_point
...
private:
    ...
    GSPLAT_HOST_DEVICE auto compute_delta(
        const glm::fvec2 xy, const float r2
    ) -> glm::fvec2 {
        auto const derived = static_cast<Derived*>(this);
        ...
        auto const delta_x =
            p1 * axy + p2 * (r2 + axx) + r2 * (s1 + r2 * s2);
        auto const delta_y =
            p2 * axy + p1 * (r2 + ayy) + r2 * (s3 + r2 * s4);
        return glm::fvec2{delta_x, delta_y};
    }

    // Compute the Jacobian of the shifting distortion: d(delta) / d(xy)
    GSPLAT_HOST_DEVICE auto jacobian_delta(
        const glm::fvec2 xy, const float r2
    ) -> glm::fmat2 {
        auto const derived = static_cast<Derived*>(this);
        auto const &[p1, p2] = derived->get_tangential_coeffs();
    ...
    GSPLAT_HOST_DEVICE auto compute_distortion(
        const glm::fvec2 &xy
    ) -> std::pair<glm::fvec2, bool> {
    ...
    GSPLAT_HOST_DEVICE auto jacobian_distortion(
        const glm::fvec2 &xy
...
struct OpencvPinholeProjection : OpencvPinholeProjectionImpl<OpencvPinholeProjection> {
    ...
    GSPLAT_HOST_DEVICE auto get_principal_point() const { return principal_point; }
    GSPLAT_HOST_DEVICE auto get_radial_coeffs() const { return radial_coeffs; }
    GSPLAT_HOST_DEVICE auto get_tangential_coeffs() const { return tangential_coeffs; }
    GSPLAT_HOST_DEVICE auto get_thin_prism_coeffs() const { return thin_prism_coeffs; }
};

struct BatchedOpencvPinholeProjection : OpencvPinholeProjectionImpl<BatchedOpencvPinholeProjection> {
    uint32_t n{0}, idx{0};                                                 
