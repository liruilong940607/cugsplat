...
inline GSPLAT_HOST_DEVICE glm::fvec3 forward_substitution(const glm::fmat3& L, const glm::fvec3& y) {
    constexpr float eps = 1e-10f;
    glm::fvec3 x;
    x[0] = y[0] / L[0][0];
    x[1] = (y[1] - L[0][1] * x[0]) / (std::abs(L[1][1]) > eps ? L[1][1] : eps);
    x[2] = (y[2] - L[0][2] * x[0] - L[1][2] * x[1]) / (std::abs(L[2][2]) > eps ? L[2][2] : eps);
    return x;
}
#define cholesky_Linv_y forward_substitution

// Solve Lᵀx = y where L is lower triangular
inline GSPLAT_HOST_DEVICE glm::fvec3 backward_substitution(const glm::fmat3& L, const glm::fvec3& y) {
    constexpr float eps = 1e-10f;
    glm::fvec3 x;
...
inline GSPLAT_HOST_DEVICE glm::fvec3 cholesky_Winv_y(const glm::fmat3& L, const glm::fvec3& y) {
...
inline GSPLAT_HOST_DEVICE glm::fmat3 cholesky_Linv_vjp(const glm::fmat3& L, const glm::fmat3& v_Linv) {
    glm::fmat3 X;
    for (int c = 0; c < 3; ++c) {
    ...
    for (int r = 0; r < 3; ++r) {
        ...
        G[0][r] = -solved[0]; G[1][r] = -solved[1]; G[2][r] = -solved[2];
    }
    return G;
}
...
inline GSPLAT_HOST_DEVICE glm::fmat3 cholesky_vjp(const glm::fmat3& L, const glm::fmat3& v_L) {
