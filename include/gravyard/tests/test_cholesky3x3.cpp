...
void test_forward_substitution() {
    ...
    glm::fvec3 x = cugsplat::forward_substitution(L, y);
...
void test_backward_substitution() {
    ...
    glm::fvec3 x = cugsplat::backward_substitution(L, y);
...
void test_forward_substitution_vjp() {
    ...
    glm::fmat3 v_L = cugsplat::forward_substitution_vjp(L, x, v_x);
    assert(v_L[0][0] < 0);
}
...
void test_cholesky_Winv_y() {
    ...
    assert(ok && "cholesky decomposition failed");
    glm::fvec3 y(1.0f);
    glm::fvec3 x = cugsplat::cholesky_Winv_y(L, y);
...
void test_cholesky_Winv() {
    ...
    auto [L, ok] = cugsplat::cholesky(A);
    assert(ok && "cholesky decomposition failed");
    glm::fmat3 Winv = cugsplat::cholesky_Winv(L);
...
void test_cholesky_Linv() {
    ...
    auto [L, ok] = cugsplat::cholesky(A);
    assert(ok && "cholesky decomposition failed");
    glm::fmat3 Linv = cugsplat::cholesky_Linv(L);
...
//     assert(ok && "cholesky decomposition failed");
//     glm::fmat3 Linv = cugsplat::cholesky_Linv(L);
//     glm::fmat3 v_L = cugsplat::cholesky_Linv_vjp(L, v_Linv);
...
//     auto [L, ok] = cugsplat::cholesky(A);
//     assert(ok && "cholesky decomposition failed");
//     glm::fmat3 v_A = cugsplat::cholesky_vjp(L, v_L);
